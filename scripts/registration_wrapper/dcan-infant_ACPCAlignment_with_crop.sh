...
# take neck crop to final output
${FSLDIR}/bin/fslmaths "$WD"/robustroi -add 1 -bin "$WD"/roimask
${FSLDIR}/bin/flirt -interp nearestneighbour -in "$WD"/roimask -ref "${Input}" -applyxfm -init "$WD"/roi2full.mat -out "$WD"/roimask
${FSLDIR}/bin/applywarp --rel --interp=nn -i "$WD"/roimask -r "$Reference" --premat="$OutputMatrix" -o "$WD"/roimask
${FSLDIR}/bin/fslmaths "${Output}" -mas "$WD"/roimask "${Output}"


# transform head with OutputMatrix to also transform to acpc alignment.
#${FSLDIR}/bin/flirt -in "$Input" -ref "$Reference" -applyxfm -init "$OutputMatrix" -out "$Output"
